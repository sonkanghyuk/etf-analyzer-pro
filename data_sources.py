...
def download_prices(tickers: list[str], start, end) -> tuple[pd.DataFrame, pd.DataFrame]:
    """Download adjusted and close prices robustly for one or many tickers."""
    if not tickers:
    ...
    # Recent yfinance versions commonly return MultiIndex columns even for one ticker.
    if isinstance(raw.columns, pd.MultiIndex):
        ...
        closes = raw["Close"].copy() if "Close" in fields else prices.copy()
        ...
        prices = raw[[price_field]].copy()
        closes = raw[["Close"]].copy() if "Close" in raw.columns else prices.copy()
        prices.columns = [tickers[0]]
        closes.columns = [tickers[0]]

    requested = [t for t in tickers if t in prices.columns]
...
def fetch_fund_snapshot(ticker: str) -> dict:
    ...
    try:
        ...
        # Metadata can occasionally be throttled; failures here should not break the app.
        try:
            info = tk.info or {}
